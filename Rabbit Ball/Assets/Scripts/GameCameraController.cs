...
public class GameCameraController : MonoBehaviour {
	...
	void Update () {
		if (target) {
			...
			transform.position = Vector3.Lerp(transform.position, target.transform.position - target.transform.forward * targetDistance + Vector3.up * 4f, Time.deltaTime * followSpeed);
